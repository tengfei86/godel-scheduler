...
func TestPodMatchesNodeSelectorAndAffinityTerms(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			err: ErrNodeSelectorNotMatching,
		},
		{
			name: "same labels",
			pod: &v1.Pod{
		...
		},
		{
			...
			err:  ErrNodeAffinityNotMatching,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			p, _ := New(nil, nil)
			p.(framework.PreFilterPlugin).PreFilter(context.Background(), state, test.pod)
			nodeAffinityRelatedState, _ := getPreFilterState(state)
			err := podMatchesNodeSelectorAndAffinityTerms(test.pod, nodeAffinityRelatedState, node.Labels, node.Name)
			if test.err == nil && err == nil {
				return
			}
			if test.err != nil && err == nil {
				t.Errorf("node selector and affinity terms matched")
			} else if test.err == nil && err != nil {
				t.Errorf("node selector or affinity terms not matched")
