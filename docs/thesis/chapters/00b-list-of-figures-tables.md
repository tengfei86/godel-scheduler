...
- 图 6-3  各场景有效吞吐对比（总工作量 / 总完成时间，3 次重复取中位数）
- 图 6-4  P90 调度延迟对比（s3, w3，中位数聚合，n=3）
- 图 6-5  P99 调度延迟对比（s3, w3，中位数聚合，n=3）
- 图 6-6  绑定成功率对比（s3, w3，中位数聚合，n=3）
- 图 6-7  Pod E2E 延迟 P99 对比（s3, w3，中位数聚合，n=3）
- 图 6-8  峰值吞吐对比（s3, w3，中位数聚合，n=3）
- 图 6-9  bind_inflight 绑定并发度对比（s3, w3，中位数聚合，n=3）
- 图 6-10  P99 调度延迟对比（s2, w3，中位数聚合，n=3）
- 图 6-11  五调度器稳态吞吐对比（s4, w3, inst3，中位数聚合，n=3）
- 图 6-12  五调度器 P99 调度延迟对比（s4, w3, inst3，中位数聚合，n=3）
- 图 6-13  五调度器稳态吞吐对比（s3, w4 极限负载, inst3，中位数聚合，n=3）
- 图 6-14  五调度器稳态吞吐对比（s4, w5 突发洪峰, inst3，中位数聚合，n=3）
- 图 6-15  五调度器稳态吞吐对比（s3, w3, inst3，中位数聚合，n=3）
...
> 说明：本清单由各章图题与表题自动汇总，共 27 张插图、12 张附表。
