...
show_node_partition() {
  ...
  # 检测当前部署的命名空间（godel-system 或 eno-system）
  local active_ns="${ENO_NAMESPACE}"
  local anno_domain="eno.io"
  if kubectl get deployment -n "${GODEL_NAMESPACE}" --no-headers 2>/dev/null | grep -q "scheduler"; then
    active_ns="${GODEL_NAMESPACE}"
    anno_domain="godel.bytedance.com"
  fi

  local schedulers
  # 支持两种命名方式：scheduler / scheduler-0,1,2
  schedulers=$(kubectl get deployment -n "${active_ns}" --no-headers 2>/dev/null \
  ...
  annotated_count=$(kubectl get nodes -o json 2>/dev/null | \
    jq --arg key "${anno_domain}/scheduler-name" '[.items[] | select(.metadata.annotations[$key] != null)] | length')
  ...
  for sched in $schedulers; do
    # 物理分区注解值是 Scheduler CRD 名称（如 godel-scheduler / eno-scheduler），不是 deployment 名
    ...
    for sname in ${sched_names:-$sched}; do
      ...
      count=$(kubectl get nodes -o json 2>/dev/null | \
        jq --arg key "${anno_domain}/scheduler-name" --arg val "$sname" \
        "[.items[] | select(.metadata.annotations[\$key]==\$val)] | length")
      if (( count > 0 )); then
