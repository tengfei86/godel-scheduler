...
在机制层面，本文提出基于 etcd 语义的分层绑定容错策略。构建四层结构化容错链路，即节点分区验证（Layer 0，预防层）、同步指数退避重试（Layer 1，即时恢复层）、异步 Reconciler 队列（Layer2，后台恢复层）与 Dispatcher 跨实例回退（Layer3，全局恢复层），并给出核心一致性约束"任一 Pod 至多绑定到一个节点"的证明要点（P1~P4）。
