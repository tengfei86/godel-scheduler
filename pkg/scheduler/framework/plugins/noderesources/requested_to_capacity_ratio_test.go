...
func (f *fakeData) Clone() framework.StateData {
	copy := &fakeData{
		data: f.data,
	}
	return copy
...
func TestRequestedToCapacityRatio(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if err := framework.SetPodResourceTypeState(podutil.GuaranteedPod, state); err != nil {
				t.Errorf("cycle state error: %v", err)
			}
			...
			for _, p := range test.scheduledPods {
				cache.AddPod(p)
			}
