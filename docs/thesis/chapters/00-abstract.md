...
（1）单 Dispatcher、多独立 Scheduler 分布式调度架构（ENO）。构建"单 Dispatcher 统一分发 + 多 Scheduler 独立并行执行"的分发-执行解耦协同模型，将 Binder 绑定执行链路与 Scheduler 决策实例同域化，消除了跨组件的 gRPC/Informer 事件传递与序列化开销，使系统整体吞吐能力可随 Scheduler 实例数近线性扩展。设计并实现了 CacheAdapter 共享缓存适配层，使 Scheduler 与进程内 Binder 通过对象引用共享同一份 SchedulerCache 内存实例，实现零拷贝、零同步延迟的状态复用。
...
实验评估。在基于 KWOK 的仿真集群上（覆盖 1000~10000 节点共 5 档集群规模、7 类工作负载与 1/3 两种实例配置），与 Gödel、kube-scheduler、Volcano、Koordinator 四个基线进行了系统性对比，共完成两个批次 126 次基准实验。实验结果表明：本文提出的分布式调度架构在 1000 pods/s 高负载下稳态调度吞吐较 Gödel 提升 6.6%~13.7%（峰值吞吐提升 13.3%~39.0%），P99 调度延迟降低 47.5%~82.0%，Pod E2E 延迟降低 40.7%~91.0%；在 10000 节点规模下 P99 调度延迟较 Gödel 降低 97.1%（0.21 s 对 7.29 s）；实例数由 1 增至 3 时峰值吞吐提升 61.0%；分层容错机制保障了 100% 的绑定成功率（126 次实验无一失败），且始终维持核心一致性不变量。相比 kube-scheduler、Volcano、Koordinator 等单实例调度器，本文方案在大规模场景下展现出显著的水平扩展性优势。
...
Evaluation. On KWOK-based simulated clusters (5 cluster scales from 1,000 to 10,000 nodes, 7 workload types, and 1/3-instance configurations), we systematically compared ENO against Gödel, kube-scheduler, Volcano, and Koordinator, completing 126 benchmark runs in two batches. Experimental results show that under high load (1,000 pods/s), ENO improves steady-state throughput over Gödel by 6.6%–13.7% (13.3%–39.0% for peak throughput), reduces P99 scheduling latency by 47.5%–82.0%, and reduces end-to-end Pod latency by 40.7%–91.0%; at the 10,000-node scale, P99 scheduling latency is reduced by 97.1% (0.21 s vs. 7.29 s); increasing the number of scheduler instances from 1 to 3 raises peak throughput by 61.0%. The layered fault-tolerance mechanism maintains a 100% binding success rate (no failure in 126 runs) and always preserves the core consistency invariant. Compared with single-instance schedulers such as kube-scheduler, Volcano, and Koordinator, the proposed scheme exhibits significant horizontal scalability advantages in large-scale scenarios.
