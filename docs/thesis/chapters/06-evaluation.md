...
若以峰值吞吐（`scheduling_peak_throughput`）衡量，ENO 在多数场景同样领先。图 6-19 汇总了 15 个有效场景的峰值吞吐对比（16 个场景中 s1/w1 因单次 run 过短、剔除首尾后无有效采样，不参与该指标对比）。ENO 领先的场景 12 个，提升幅度为 0.2%~68.0%，其中 s4/w6（inst3）达 +68.0%（893.2 对 531.6 pods/s）、s4/w7（inst3）+14.9%、s2/w3 +13.4%、s3/w3（inst1）+14.0%；Gödel 略领先的场景 2 个：s3/w5（inst3，-10.4%）与 s3/w6（inst3，-2.0%）；s4/w3（inst3）两者基本持平（-0.3%）。ENO 未在峰值吞吐上一律领先说明其有效吞吐的整体优势并非源自更高的瞬时调度速率，而是来自更快的启动进入稳态与更短的整体完成时间——这一口径差异在 §6.8 第（7）条中作为局限进一步说明。

![图 6-19  ENO 与 Gödel 峰值吞吐 15 场景对比（`scheduling_peak_throughput`，中位数聚合，n=3）](../figures/fig6-19-peak-throughput-a-vs-b-all.png)
...
图 6-12 与图 6-13 分别给出该场景（s4/w3，inst3）下 ENO 与 Gödel 的调度吞吐与 P99 调度延迟对比。

![图 6-12  ENO 与 Gödel 调度吞吐对比（s4, w3, inst3，中位数聚合，n=3）](../figures/fig6-12-throughput-s4-w3-inst3.png)

![图 6-13  ENO 与 Gödel P99 调度延迟对比（s4, w3, inst3，中位数聚合，n=3）](../figures/fig6-13-latency-p99-s4-w3-inst3.png)
...
![图 6-16  ENO 与 Gödel 调度吞吐对比（s3, w3, inst3，中位数聚合，n=3）](../figures/fig6-16-throughput-s3-w3-inst3.png)
...
图 6-14 与图 6-15 分别给出 s3/w4（极限负载）与 s4/w5（突发洪峰）场景下 ENO 与 Gödel 的调度吞吐对比。

![图 6-14  ENO 与 Gödel 调度吞吐对比（s3, w4 极限负载, inst3，中位数聚合，n=3）](../figures/fig6-14-throughput-s3-w4-inst3.png)

![图 6-15  ENO 与 Gödel 调度吞吐对比（s4, w5 突发洪峰, inst3，中位数聚合，n=3）](../figures/fig6-15-throughput-s4-w5-inst3.png)
...
**观测指标**（表 6-9 列出各指标的 PromQL 与观测预期）：
