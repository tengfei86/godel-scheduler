...
func (s *PodStore) AssumePod(podInfo *framework.CachePodInfo) error {
	...
	// Set Deadline for binder.
	dl := time.Now().Add(AssumedPodExpiration)
	s.PodStates[key] = &framework.CachePodState{Pod: podInfo.Pod, Deadline: &dl}
...
func (s *PodStore) podOp(pod *v1.Pod, isAdd bool) error {
	...
	if isAdd {
		...
		_, ok := s.PodStates[key]
		if !ok {
			return fmt.Errorf("pod %v is not found in binder cache, so cannot be removed from it", key)
		}
		...
		// 	return fmt.Errorf("pod %v is assumed in binder cache, so cannot be removed from it", key)
		// }
		delete(s.PodStates, key)
		delete(s.AssumedPods, key)
	}
