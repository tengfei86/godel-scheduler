...
// {subCluster, priority, qos, status}
...
func (c *collector) emit() {
	...
	for tag, v := range infos {
		reservationInfoCount.WithLabelValues(tag[0], tag[1], tag[2], tag[3]).Set(float64(v))
	}

	for tag, v := range c.expiredReservationInfos {
		expiredReservationInfoCount.WithLabelValues(tag.subCluster, tag.qos, tag.priority).Add(float64(v))
	}
}
