...
本文实验以 w2（中负载）与 w3（高负载）两组稳态负载为主，用于 ENO 与 Gödel 的定量对比；并补充评估 w1（低负载）、w4（极限负载）、w5（突发洪峰）与 w7（异构资源）四组场景，以覆盖更复杂的负载形态。w6（Gang 调度）与 w8 因数据未完整采集，留作未来工作（见 6.8 节）。
...
结论：在全部 16 个场景中，ENO 的有效吞吐在 14 个场景高于 Gödel、2 个场景持平（s1/w1 与 s4/w7，差异为 0.0%），提升幅度为 +1.3%~+34.5%，平均 +14.5%。所有场景的调度完成率均为 100%，说明差异来自完成时间而非完成度。提升在高负载与多实例配置下最为明显：s3/w6（inst3）为 34.5%，s3/w6（inst1）为 26.7%，s2/w3 为 22.9%，s3/w3（inst3）与 s3/w5（inst3）分别为 21.7% 与 21.3%。低负载场景（s1/w1、s2/w2）两者接近，原因是负载未使调度器饱和，完成时间主要由负载注入速率决定。
...
表 6-5 给出 ENO 与 Gödel 的调度延迟（秒，聚合序列剔除首尾各 2 个采样点后的中位数，口径见 6.3 节）：

: 表 6-5  ENO 与 Gödel 调度延迟对比（秒）

| 场景（实例配置） | ENO P90 | Gödel P90 | P90 改善 | ENO P99 | Gödel P99 | P99 改善 |
|---|---|---|---|---|---|---|
| s2/w2（inst1） | 0.034 | 0.029 | -17.5% | 0.149 | 0.112 | -32.5% |
...
结论：在除 s2/w2 之外的全部场景中，ENO 的 P90 与 P99 调度延迟均低于 Gödel，P90 改善 4.7%~79.5%、P99 改善 0.3%~82.9%。高负载场景的改善最为明显：s2/w3 的 P99 由 23.48 s 降至 4.00 s（降低 82.9%），s3/w3（inst1）由 32.57 s 降至 16.22 s（降低 50.2%），s3/w4（inst3）降低 49.7%，s4/w3（inst3）降低 33.4%。多实例配置下两者的绝对延迟都进入亚秒级（s3/w3 inst3：ENO 0.13 s、Gödel 0.15 s），说明增加实例数能同时缓解两种架构的排队压力，此时差距收窄。s2/w2 是唯一的例外：该场景未使调度器饱和，ENO 的 P99 为 0.149 s、Gödel 为 0.112 s，绝对差 0.037 s，属低负载下的微小差异，不影响高负载场景的结论方向。
...
结论：在全部 12 个已测场景中，ENO 的 Pod E2E P99 延迟都不高于 Gödel，改善幅度 0.9%~75.3%，其中 s2/w3 降低 75.3%、s3/w7（inst3）降低 68.2%、s3/w5（inst3）降低 53.8%、s3/w3（inst3）降低 51.9%。未饱和的 s2/w2 是唯一差距落在 1% 以内的场景，两者分别为 0.238 s 与 0.240 s，绝对差 0.002 s。E2E 延迟直接包含 Scheduler 与 Binder 之间的协作开销，因此这一结果量化验证了 ENO 消除 Scheduler → Binder 事件传递环节（第 5 章 5.1 节步骤 ④）对端到端时延的收益。
...
> 数据质量说明：w6 场景的延迟分位指标未能采集到有效数据（仅完成时间可用），因此该场景只报告吞吐结论；s3/w4 的元数据缺少调度完成率字段（不影响完成时间与吞吐计算）。相关缺口见 6.8 节。
...
数据说明：goroutines 指标在 ENO 与 Gödel 两组的 `avg/` 汇总中记录不完整（部分场景缺失），且无法横跨 Scheduler 与 Binder 两个 Deployment 累加，难以得到公平的全系统对比，故本节不以 goroutines 作为主要对比指标（局限见 6.8 节）。
...
（2）规模上限止于 s4：本文已覆盖 s1~s4（最大 10000 节点），但未覆盖 s5（30000 节点）场景。Gödel 官方报告在 30K 节点下运行良好，但本文暂无独立数据支撑该规模；在 s4 场景下 ENO 已展现出明显优于 Gödel 的扩展性（6.5.1 节），s5 规模的验证留作未来工作。
...
（4）规模增长未同步增加实例数：本章 s3 与 s4 的对比均固定使用 3 个实例，规模翻倍时未同步扩充实例，因此 6.5.1 节反映的是"实例数不变、规模增长"的情形，ENO 在 s4 的吞吐回落与该设定有关。若按规模比例增加实例，其扩展性表现可能更好，这一点留作后续验证。
...
（6）Volcano 指标口径的差异：Volcano 的 `volcano_task_scheduling_latency_milliseconds` 与其他调度器的 `scheduler_scheduling_attempt_duration_seconds` 在语义上并不完全等价，本文在 6.1.2 节中通过 recording rules 尽力对齐了口径，但仍难以做到 100% 严格等价的对比。这在结论中会予以说明。
...
- 吞吐（有效吞吐 = 总工作量／总完成时间）：ENO 在 16 个场景中的 14 个高于 Gödel、2 个持平，提升幅度 1.3%~34.5%、平均 14.5%，全部场景的调度完成率均为 100%；提升在 Gang 调度（w6，+22.6%~34.5%）、高负载（w3，+5.1%~22.9%）与多实例配置下最明显。以峰值吞吐衡量，ENO 在多数场景同样领先（+13.4%~14.9%），但在 s3/w5 与 s4/w3 场景下不高于 Gödel，说明其有效吞吐优势部分来自更短的整体完成时间；
- 调度延迟：除未饱和的 s2/w2 场景（P99 绝对差 0.037 s）外，ENO 的 P90/P99 调度延迟均低于 Gödel，改善幅度分别为 4.7%~79.5% 与 0.3%~82.9%；Pod E2E 延迟在全部场景都不高于 Gödel，改善幅度 0.9%~75.3%。多实例配置下两者的绝对延迟都进入亚秒级，差距收窄；
