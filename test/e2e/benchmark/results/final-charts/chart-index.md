...
指标总结: 该图按全采样点连线并叠加 rolling mean(5) 展示；按非零区间均值统计，Group B 吞吐量相对 Group A 提升约 26.7%（W3, s3, run1）。 数据缺失: D (Volcano)。
数据源:
...
指标总结: Group B 的 P99 延迟相对 Group A 下降约 23.2%（W3, s3, run1）。 数据缺失: D (Volcano)。
数据源:
...
指标总结: Group B 在绑定延迟分位（P50/P90/P99）上平均较 Group A 降低约 23.3%。
数据源:
...
指标总结: Group B 的 Pending 峰值相对 Group A 下降约 7.4%，队列堆积更轻。
数据源:
...
指标总结: Group B 在 W6 的完成时间相对 Group A 缩短约 24.2%。
数据源:
...
指标总结: 该图按全采样点连线并叠加 rolling mean(5) 展示（不做点位平均）；在 W3 聚合口径下，Group B 采样均值相对 Group A 提升约 19.3%。
数据源:
...
指标总结: Group B 在节点 CPU 利用率分布上相对 Group A 更集中，表现出更好的均衡性。
数据源:
