...
Evaluation was conducted on KWOK-based simulated clusters covering five scales (1,000–10,000 nodes), seven workloads and two instance configurations, comprising 126 benchmark runs in two batches against Gödel, kube-scheduler, Volcano and Koordinator. Under high load (1,000 pods/s), ENO improves steady-state throughput over Gödel by 6.6%–13.7% (13.3%–39.0% for peak throughput), reduces P99 scheduling latency by 47.5%–82.0%, and reduces end-to-end Pod latency by 40.7%–91.0%; at the 10,000-node scale, P99 scheduling latency is reduced by 97.1% (0.21 s vs. 7.29 s); increasing scheduler instances from 1 to 3 raises peak throughput by 61.0%. The binding success rate is 100% across all evaluated scenarios, and the core consistency invariant is always preserved. Compared with single-instance schedulers, the proposed scheme exhibits clear horizontal scalability advantages at large scale.
