...
func init() {
	// if we use nil here, coverage test with option -coverpkg will fail
...
func newTestBinder(t *testing.T, stopCh <-chan struct{}) *testEnv {
	...
	for v, synced := range informerFactory.WaitForCacheSync(stopCh) {
		if !synced {
			klog.ErrorS(nil, "Failed to sync informer", "informerType", v)
...
func makeTestPVC(name, size, node string, pvcBoundState int, pvName, resourceVersion string, className *string) *v1.PersistentVolumeClaim {
	...
	case pvcSelectedNode:
		util.SetMetaDataMap(&pvc.ObjectMeta, pvutil.AnnSelectedNode, node)
		// don't fallthrough
	case pvcBound:
...
func TestFindPodVolumesWithoutProvisioning(t *testing.T) {
	...
	run := func(t *testing.T, scenario scenarioType) {
		...
		// Execute
		reasons, err := testEnv.binder.FindPodVolumes(scenario.pod, testNode.GetName(), testNode.GetLabels())
...
func TestFindPodVolumesWithCSIMigration(t *testing.T) {
	...
	run := func(t *testing.T, scenario scenarioType) {
		...
		// Execute
		reasons, err := testEnv.binder.FindPodVolumes(scenario.pod, node.GetName(), node.GetLabels())
...
func TestFindAssumeVolumes(t *testing.T) {
	...
	// Execute
	// 1. Find matching PVs
	reasons, err := testEnv.binder.FindPodVolumes(pod, testNode.GetName(), testNode.GetLabels())
	if err != nil {
