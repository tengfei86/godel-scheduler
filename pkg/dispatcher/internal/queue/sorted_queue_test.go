...
func TestSortedFIFO(t *testing.T) {
	tests := []struct {
		name string
		ops  []operation
		want []string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for len(fifo.fifo.items) > 0 {
				infos, _ := fifo.PopPodInfo()
				got = append(got, parsePodKey(infos.PodKey))
			}

			for _, singleOp := range tt.ops {
				info := makeQueuedPodInfo(singleOp.key)
				if existing := fifo.PodInfoExist(info); existing {
