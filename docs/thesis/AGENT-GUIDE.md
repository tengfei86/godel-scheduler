...
| 日期 | 提出人 | 规则变更 | 影响范围 |
|---|---|---|---|
| 2026-09-15 | 作者 | **吞吐主口径改为「有效吞吐 = 总工作量／总完成时间」**：数据取自各 run 的 `metadata.txt`（`total` 与 `duration`），每条 run 独立计算后取 **3 次中位数**；时序类指标（峰值吞吐、延迟分位、队列堆积）先按时间点对 3 条 run 取中位数聚合，再在**有效样本**（非 NaN 采样点）上取中位数；**不再使用"剔除头尾 30 秒的窗口均值"口径**——该口径受 run 时长影响，且被负载结束后的零值稀释（实测 s2/w3 的 Gödel 窗口均值为 342 而平台值约 780）。数据源统一为 `test/e2e/benchmark/results/compare/` 下的 16 个对比场景 | 第 6 章 + 摘要 + 附录 A |
