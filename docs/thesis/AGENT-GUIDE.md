...
| 日期 | 提出人 | 规则变更 | 影响范围 |
|---|---|---|---|
| 2026-09-15 | 作者 | **w6（Gang）延迟数据将重新采集**（数据由作者自行重跑）。当前 w6 延迟为无数据的根因已定位：w6 单次 run 仅 29~39 s，而延迟分位依赖 `rate(..._bucket[1m])` 的 1 分钟窗口，导出序列只有 0~1 个采样点，按 §6.3 口径（去首尾各 2 点）自然为空。因此单纯重跑同一负载仍会得到 n/a，需要在**采集侧**改为按 run 累计直方图取分位（用 Step 9 与 Step 6 两次快照的 `_bucket` 计数之差做 `histogram_quantile`），或把 w6 负载时长拉长到 ≥ 3 分钟。数据到位后需同步：§6.3 中"w6 无数据"的举例、表 6-5/表 6-6（新增 w6 行）、§6.5.3、§6.8 第（3）条、§6.9、附录 A、图表清单；另 `results/a/s3/w6/inst1/avg` 一直缺失（原始 run 数据齐全，仅未生成聚合），需一并补跑 `gen-averages.sh --median` | 第 6 章 + 摘要 + 附录 A |
