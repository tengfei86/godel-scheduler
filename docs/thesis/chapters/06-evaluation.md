...
结论：在 w3 高负载（1000 pods/s）场景下，ENO 稳态吞吐较 Gödel 提升 6.6%~13.7%，峰值吞吐提升 13.3%~39.0%，且该优势在规模扩大（s4）与实例增多（inst3）时保持甚至扩大。在 w2 中负载（500 pods/s）场景下，ENO 的稳态吞吐数值低于 Gödel（-23%~-24%），但该场景下 ENO 的调度延迟与队列堆积均显著更低（§6.4.2、§6.5），说明两者的差异主要来自调度节奏——ENO 通过更低的队列等待换取更平稳的完成节奏，而非处理能力不足；事实上在 s3/w2 场景下 ENO 的峰值吞吐反而更高（632.2 对 500.6，+26.3%）。综合而言，ENO 的吞吐优势主要体现在系统饱和的高负载场景，这也正是架构改造要解决的目标场景。
