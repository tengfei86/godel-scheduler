...
func MaxRevision(allRSs []*apps.ReplicaSet) int64 {
	...
	for _, rs := range allRSs {
		if v, err := Revision(rs); err != nil {
			...
			klog.InfoS("Failed to parse revision for the ReplicaSet, deployment controller will skip it when reconciling revisions", "replicaSet", klog.KObj(rs), "err", err)
		} else if v > max {
			max = v
		}
	}
...
func SetNewReplicaSetAnnotations(deployment *apps.Deployment, newRS *apps.ReplicaSet, newRevision string, exists bool, revHistoryLimitInChars int) bool {
	...
	if err != nil {
		if oldRevision != "" {
			klog.InfoS("WARN: OldRevision was not an int type", "err", err)
			return false
		}
	...
	if err != nil {
		klog.InfoS("WARN: NewRevision was not an int type", "err", err)
		return false
	}
	if oldRevisionInt < newRevisionInt {
		newRS.Annotations[RevisionAnnotation] = newRevision
		annotationChanged = true
		klog.V(4).InfoS("Updated ReplicaSet revision", "replicaSet", klog.KObj(newRS), "revision", newRevision)
	}
	...
	if ok && oldRevisionInt < newRevisionInt {
		...
		if len(oldRevisions[0]) == 0 {
			...
			if totalLen <= revHistoryLimitInChars {
				...
				klog.InfoS("WARN: Did not append revision due to length limit reached", "lengthLimit", revHistoryLimitInChars)
			}
		}
	}
...
func getIntFromAnnotation(rs *apps.ReplicaSet, annotationKey string) (int32, bool) {
	...
	if err != nil {
		klog.InfoS("Failed to convert the annotation for the ReplicaSet", "annotationKey", annotationKey, "annotationValue", annotationValue, "replicaSet", klog.KObj(rs), "err", err)
		return int32(0), false
	}
...
func DeploymentTimedOut(deployment *apps.Deployment, newStatus *apps.DeploymentStatus) bool {
	...
	klog.V(4).InfoS("Deployment timed out", "deployment", klog.KObj(deployment), "timedOut", timedOut, "lastProgress", from, "currentProgress", now)
	return timedOut
}
