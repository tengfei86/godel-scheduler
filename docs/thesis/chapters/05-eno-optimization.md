...
第 3、4 章构建的分布式 Kubernetes 调度器架构在正确性与容错性上已经完备，但性能仍然存在优化空间。特别是原架构中独立部署的 Binder 组件，在超高并发场景下会重新成为整个调度流水线的瓶颈。本章提出 ENO（Embedded Node Operator）——将 Binder 合并进 Scheduler 进程的架构改造方案，通过 Cache 零拷贝共享、消除跨进程 API 调用、保持 CLI 开关可切换性等设计要点，在不牺牲一致性保证的前提下取得显著的性能提升。
