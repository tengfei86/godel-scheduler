...
type pdbItemImpl struct {
	...
	replicaSetKeys GenerationStringSet
	daemonSetKeys  GenerationStringSet
	generation     int64
}

var (
	_ PDBItem                   = &pdbItemImpl{}
	_ generationstore.StoredObj = &pdbItemImpl{}
)

func NewPDBItemImpl(pdb *policy.PodDisruptionBudget) *pdbItemImpl {
	return &pdbItemImpl{pdb, nil, NewGenerationStringSet(), NewGenerationStringSet(), 0}
}
...
func (p *pdbItemImpl) RemoveAllOwners() {
	p.replicaSetKeys = NewGenerationStringSet()
	p.daemonSetKeys = NewGenerationStringSet()
}
...
func (p *pdbItemImpl) Replace(item PDBItem) {
	if item == nil {
		return
	}
	p.pdb = item.GetPDB()
	p.pdbSelector = item.GetPDBSelector()
	p.daemonSetKeys.Reset(NewGenerationStringSet(item.GetRelatedOwnersByType(util.OwnerTypeDaemonSet)...))
	p.replicaSetKeys.Reset(NewGenerationStringSet(item.GetRelatedOwnersByType(util.OwnerTypeReplicaSet)...))
	p.generation = item.GetGeneration()
}

func (p *pdbItemImpl) Clone() PDBItem {
	return &pdbItemImpl{
		pdb:            p.pdb.DeepCopy(),
		pdbSelector:    p.pdbSelector.DeepCopySelector(),
		generation:     p.generation,
		replicaSetKeys: NewGenerationStringSet(p.replicaSetKeys.Strings()...),
		daemonSetKeys:  NewGenerationStringSet(p.daemonSetKeys.Strings()...),
	}
}
