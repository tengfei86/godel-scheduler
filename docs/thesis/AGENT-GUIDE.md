...
| 2026-08-27 | 本人 | 新增 `docs/thesis/interim-report/中期报告.md` 为中期报告 Markdown 源文件：数据与论文第 6 章同口径（稳态窗口均值，去除头尾 30s），图表引用 `figures/` 现成图；Word 草稿 docx 中的"中期检查口径"（非零区间均值，如吞吐 +26.7%）与之不同，**提交前须统一口径** | 中期报告 |
