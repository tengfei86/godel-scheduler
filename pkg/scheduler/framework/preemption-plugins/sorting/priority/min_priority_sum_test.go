...
func TestMinPrioritySum(t *testing.T) {
	tests := []struct {
		name          string
			...
			candidates: []*framework.Candidate{
				{
					Victims: &framework.Victims{
						Pods: []*v1.Pod{
							testinghelper.MakePod().Name("p1").UID("p1").Node("n1").Priority(10).
								Annotation(util.CanBePreemptedAnnotationKey, util.CanBePreempted).Obj(),
				...
				},
				{
					Victims: &framework.Victims{
						Pods: []*v1.Pod{
							testinghelper.MakePod().Name("p3").UID("p3").Node("n2").Priority(10).
								Annotation(util.CanBePreemptedAnnotationKey, util.CanBePreempted).Obj(),
