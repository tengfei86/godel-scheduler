...
2. **插值对齐**：各 run 的采样点不一定完全一致（step 相同但起始时间不同），用 `numpy.interp` 将所有 run 插值到公共相对时间网格（以覆盖范围最小的 run 为基准，固定 15s step）
3. **逐点平均**：`numpy.mean(aligned_arrays, axis=0)`，计算各时间点的均值
4. **标准差 band（可选）**：`numpy.std(aligned_arrays, axis=0)`，用 `ax.fill_between` 绘制 ±1σ 半透明区域
5. **输出文件命名**：`avg_<metric_name>.<fmt>`，区别于单 run 图表

**对 peak 类指标的特殊处理**：`*peak_throughput*` 类指标，额外在图上用 `ax.axhline` + `ax.text` 标注各 run 末尾值的均值±std，提供标量汇报。
...
当前 `METRIC_META` 缺少 `export-prometheus.sh` 中新增的若干 key，会 fallback 到空 ylabel。需补全：
...
"queue_incoming_pods":            {"title": "Queue Incoming Pods",                "ylabel": "pods/s"},
```
...
python3 plot-results.py \
  ...
  --average --std-band \
  --output charts_a_s2_w1_avg

# 先做各组平均，再做跨组对比（两步操作）
python3 plot-results.py results/a/s2/w1/run{1,2,3} --average --output avg_a
python3 plot-results.py results/b/s2/w1/run{1,2,3} --average --output avg_b
python3 plot-results.py avg_a avg_b --compare --output compare_a_vs_b
```
...
1. 文件头添加 `import numpy as np`
2. 扩充 `METRIC_META` 字典（补全上述缺失 key）
3. 新增 `average_runs(dirs, output_dir, metric_names, fmt, std_band)` 函数
...
1. 造假数据：手动创建 3 个 run 目录，各放一个 `scheduling_throughput.json`（数值不同），运行 `--average`，确认输出曲线是三者的逐点均值
2. `--std-band` 验证：用方差不为零的测试数据，确认图上有半透明 ±1σ 区域
3. 延迟分位数指标确认 title/subtitle 包含说明文字
4. peak 类指标确认末尾标量标注出现在图上
5. `METRIC_META` 补全验证：对新增 key 的 JSON 文件执行 `plot_directory`，确认 ylabel 不为空字符串
