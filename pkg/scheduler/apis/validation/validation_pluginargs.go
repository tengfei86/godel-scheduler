...
func validateWhenUnsatisfiable(p *field.Path, v v1.UnsatisfiableConstraintAction) *field.Error {
	...
	if !supportedScheduleActions.Has(string(v)) {
		return field.NotSupported(p, v, supportedScheduleActions.UnsortedList())
	}
	return nil
}
...
func validateResourcesNoMax(resources []config.ResourceSpec) error {
	for _, r := range resources {
		if r.Weight < 1 {
			return fmt.Errorf("resource %s weight %d must not be less than 1", r.Name, r.Weight)
		}
	}
	return nil
}
...
func ValidateLocalStoragePoolCheckerArgs(args *config.LocalStoragePoolCheckerArgs) error {
	if args.ScorePolicy != config.ScorePolicyMostAvailable && args.ScorePolicy != config.ScorePolicyLeastAvailable {
		return fmt.Errorf("invalid policy %v", args.ScorePolicy)
	}
	return validateResources(args.StorageClassWeights)
}

func ValidateLoadAwareArgs(args *config.LoadAwareArgs) error {
	for _, resourceSpec := range args.Resources {
		if resourceSpec.Weight == 0 {
			return fmt.Errorf("resource weight should not be zero")
		}
		if resourceSpec.ResourceType != podutil.GuaranteedPod && resourceSpec.ResourceType != podutil.BestEffortPod {
			return fmt.Errorf("resource type %v is invalid", resourceSpec.ResourceType)
		}
	}
	return nil
}
